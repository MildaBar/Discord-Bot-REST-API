...
export async function up(db: Kysely<SqliteDatabase>) {
  ...
  await db.schema
    ...
    .addColumn("sprint_id", "integer", (c) =>
      c.references("sprints.id").onDelete("cascade").notNull()
    )
    .addColumn("message_template_id", "integer", (c) =>
      c.references("message_templates.id").onDelete("cascade").notNull()
    )
    .addColumn("gif_id", "integer", (c) =>
      c.references("gifs.id").onDelete("cascade").notNull()
    )
